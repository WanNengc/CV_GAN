...
# def get_inception_score(images, device='cuda:0', splits=10):
#     """
#     计算Inception Score
#     """
...
#
#     # 将图像堆叠为一个batch
#     batch = torch.cat(image_list, dim=0)
#
#     # 获取InceptionV3网络的预测分布
#     with torch.no_grad():
...
#         part = pred[i * (len(pred) // splits): (i + 1) * (len(pred) // splits), :]
#         py = part.mean(0)
#         scores.append(np.exp(entropy(py.cpu().numpy(), part.cpu().numpy().T)))
#
...
#
#     mean_diff = (real_mean - fake_mean).norm()
#     cov_sqrt = sqrtm(real_cov @ fake_cov)
#
#     fid_score = mean_diff + np.trace(cov_sqrt)
#     return fid_score
#
#
# # Example usage
